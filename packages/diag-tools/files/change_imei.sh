...
#
# Ships alongside diag_nv_write in the diag-tools OpenWrt package, so both
# are already on-device at /usr/bin -- no manual copying needed. Override
# DIAG_NV_WRITE if you're running this ad hoc against a binary elsewhere
# (e.g. a fresh CI build copied to /tmp, before the package is on-device).
...
CHAN="$2"
IMEI="$3"
TOOL="${DIAG_NV_WRITE:-/usr/bin/diag_nv_write}"
